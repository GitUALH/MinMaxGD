...
{ serie temp;
    temp.p.init(infinity,_infinity);
...
serie& serie :: operator=(const poly& p1)	  // initialise avec un polynome cast polynome->serie
{
  ...
  q=p.getpol(p.getn()-1);
  p.pop();
  if (q.getpol(q.getn()-1).getd()!=infinity) r.init(0,0);
  else r.init(0,infinity);
  canonise=1;
  return *this;
}

serie& serie :: operator=(const gd& gd1)	  // initialise avec un monome cast monome->serie
{ p.init(infinity,_infinity);
  q=gd1;
  r.init(0,0);
  canonise=1;
return *this;
...
if (q.getpol(q.getn()-1).getd()==infinity || p.getpol(p.getn()-1).getd()==infinity )
{ p=oplus(p,q);
...
//  ** si l'une des s�ies vaut Top **/
  ...
  if (s1.q.getpol(0).getg()==infinity)   return(s2);
  if (s2.q.getpol(0).getg()==infinity)   return(s1);
...
/***** Test des cas d����	********************************/
	...
	{ if ((*ads1).r.getg()==0 ) // p=p1+p2+q2+q1*(0,infinity) q=dernier point du polyn�e r=0,0
	   {
			monome.init((*ads1).q.getpol(0).getg(),infinity); // normalement pas necessaire
			result.p=monome;
	...
	{	// p=p1+p2+q1*(0,infinity)+q2*(o,infinity)
		monome.init(s1.q.getpol(0).getg(),infinity); // normalement pas necessaire
		result.p=monome;
	    result.p=oplus(result.p,s1.p,s2.p);
		monome.init(s2.q.getpol(0).getg(),infinity);	// normalement pas necessaire
	...
	// on etend q1 au dela du premier point de q2
	  ...
	  temp=otimes(s1.q,monome);
	   ...
	   monome.init(s2.q.getpol(0).getg(),infinity); //normalemnt pas necessaire
	   result.p=oplus(temp=oplus(s2.p,s1.p,result.p),monome);
	   result=result.p;
	   result.r.init(0,infinity);	//normalement pas necessaire
	   result.canonise=1;
	...
	// on etend q2 au dela du premier point de q1
	  ...
	  {	  monome.init(s2.r.getg()*i,s2.r.getd()*i);
	   ...
	   monome.init(s1.q.getpol(0).getg(),infinity); //normalemnt pas necessaire
...
//********* r1 * r2 * *******************//
	...
	if( ( s1.r.getg()==0 && s1.r.getd()==infinity ) ||( s2.r.getg()==0 && s2.r.getd()==infinity ) )
	{   //monome.init(0,infinity);
		//temp1.q.add(monome);
		//	temp1.q.simpli();
		temp1.p.init(infinity,_infinity);
	...
	if (s1.r.getd()==0 && s1.r.getg()==0 && s2.r.getg()!=0 && s2.r.getd()!=0 && s2.r.getd()!=infinity)
	...
	if ((*ads2).r.getd()==0 && (*ads2).r.getg()==0 && (*ads1).r.getg()!=0 && (*ads1).r.getd()!=0 && (*ads1).r.getd()!=infinity)
		{
			temp1.r=(*ads1).r;
			temp1.p.init(infinity,_infinity);
			temp1.canon();
...
//*** cas particulier p2=epsilon ou s1=epsilon ***
  if (p2.getpol(0)==epsilon || s1.q.getpol(0).getg()==infinity )
  { result.p=epsilon;
	result.q=epsilon;
	result.r.init(0,0);
	result.canonise=1;
...
//*** cas particulier p2=Top **
  if (p2.getpol(0).getg()==_infinity)
  { result=s1;
    return(result);
  }
//** si s1 vaut Top

if (s1.q.getpol(0).getg()==_infinity)
{result=p2;
return(result);
}
...
if (s1.r.getd()==infinity)
{
 ...
 if (result.p.getpol(result.p.getn()-1).getd()==infinity) result.r.init(0,infinity); //p2 est �alement une trajectoire
...
if (p2.getpol(p2.getn()-1).getd() ==infinity)
{
	temp=epsilon;
	i=0;
...
//** si l'une des s�ies vaut Top
  ...
  if (s1.q.getpol(0).getg()==_infinity)   return(s2);
  if (s2.q.getpol(0).getg()==_infinity)   return(s1);
...
{ result.p=inf(temp1=oplus(s1.p,s1.q),temp2=oplus(s2.p,s2.q));
	result.q=result.p.getpol(result.p.getn()-1);
	result.p.pop();
    if (s1.r.getd()==infinity && s2.r.getd()==infinity) result.r.init(0,infinity);
	else result.r.init(0,0);
	result.canonise=1;
	return(result);
 }
...
if (s1.q.getpol(0).getg()==_infinity || s2.q.getpol(0).getg()==infinity)
  { result.p=epsilon;
	result.q=Top;
	result.r.init(0,0);
	result.canonise=1;
...
if (s1.q.getpol(0).getg()==infinity || s2.q.getpol(0).getg()==_infinity)
{ result.p=epsilon;
  result.q=epsilon;
  result.r.init(0,0);
  result.canonise=1;
...
if (s1.r.getg()>0 && s2.r.getd()==infinity)
{	result.p=epsilon;
	result.q=epsilon;
	result.r.init(0,0);
	result.canonise=1;
...
if (s1.r.getg()==0 && s1.r.getd()==infinity)
{
...
// modification 04/2011 L. hardouin
    int k11, k12;
     ...
     if (s1.p.getpol(0).getg()==infinity ||s1.p.getpol(0).getg()==_infinity)
     k12=0;
     else
...
/**** odot product : dual product used in PhD of Iteb Ouerghi and Thomas Brunsch, .... 09/04/2011 Laurent Hardouin */
...
s1.canon();
serietop.q.init(_infinity,infinity);
m1=s1.q.getpol(0);
...
//** If one series is equal to epsilon
...
//** if one series is equal to Top

  if (s1.q.getpol(0).getg()==_infinity || s2.q.getpol(0).getg()==_infinity)
  { result.p.init(_infinity,infinity);
    result.q.init(_infinity,infinity);
	result.r.init(0,0);
	result.canonise=1;
	return(result);
...
if (s1.r.getg()==0 && s2.r.getg()==0)
{ result.p=odot(temp1=oplus(s1.p,s1.q),temp2=oplus(s2.p,s2.q));
	result.q=result.p.getpol(result.p.getn()-1);
	result.p.pop();
    if (s1.r.getd()==infinity && s2.r.getd()==infinity) result.r.init(0,infinity);
	else result.r.init(0,0);
...
if (s1.r.getg()==0 )
 {  result.p=oplus(s1.p,s1.q);
	result=odot(s2,result.p);
	return(result);
 }

if (s2.r.getg()==0 )
 {  result.p=oplus(s2.p,s2.q);
	result=odot(s1,result.p);
	return(result);
 }

// Non degenerated case
//Calcul de p3=p1 odot p2

if( s1.q.getpol(0).getd()>s2.q.getpol(0).getd())
		{
			ads1=&s2;
			ads2=&s1;

		}
/* the slope computation of the resulting series */
   a=lcm(ads1->r.getd(),ads2->r.getd());
   I=a/ads1->r.getd();
   J=a/ads2->r.getd();
   result.r.init(ads1->r.getg()*I+ads2->r.getg()*J,a);


   int dmax=MAX(ads1->q.getpol(0).getd(),ads2->q.getpol(0).getd())+result.r.getd()+1;
   int dmin=MAX(ads1->q.getpol(0).getd(),ads2->q.getpol(0).getd());


    p1=ads1->p;
    q1=ads1->q;

   while(p1.getpol(p1.getn()-1).getd()<dmax)
   {
   ...
   while(p2.getpol(p2.getn()-1).getd()<dmax)
   {
...
i=0;
p3=odot(p1,p2);
while(p3.getpol(i).getd()<dmin && i<p3.getn())
{ if(i==0) result.p=p3.getpol(i);
  else result.p.add(p3.getpol(i));
  i++;
}
result.q=p3.getpol(i);
while(p3.getpol(i).getd()<dmax )
{   i++;
    if (i<p3.getn())
   result.q.add(p3.getpol(i));
...
//*** cas particulier p2=epsiloi=0;n ou s1=epsilon ***
  if (p2.getpol(0).getg()==infinity || s1.q.getpol(0).getg()==infinity )
  { result.p=epsilon;
	result.q=epsilon;
	result.r.init(0,0);
	result.canonise=1;
...
//*** cas particulier p2=Top or s1=Top**
  if (p2.getpol(0).getg()==_infinity|| s1.q.getpol(0).getg()==_infinity )
  {  result.p=Top;
    result.q=Top;
	result.r.init(0,0);
	result.canonise=1;
...
//*** si s1 est un polynome
if (s1.r==e)
{
 result.p=oplus(s1.p,s1.q);
 result.p=odot(result.p,p2);
 result.q=result.p.getpol(result.p.getn()-1);
 result.p.pop();
 result.r.init(0,0);
 result.canonise=1;
...
if (s1.r.getd()==infinity)
{
 ...
 if (result.p.getpol(result.p.getn()-1).getd()==infinity) result.r.init(0,infinity); //p2 est egalement une trajectoire
 else result.r.init(0,0);
 result.canonise=1;
 return(result);
}

/*** Calcul de p1 \odot p2 **/

result.p=odot(s1.p,p2);
...
if (p2.getpol(p2.getn()-1).getd() ==infinity)
{
	...
	result.p=oplus(result.p,temp2=odot(p2,temp));
	monome.init(i*s1.r.getg(),i*s1.r.getd());
	result.q=otimes(s1.q,monome);
	result.q=odot(s1.q,p2.getpol(p2.getn()-1)); // on ne fait le odot que sur le dernier élement
	result.r=s1.r;
	result.canon();
	return(result);
}
...
//*** s1 is a series  and p2 is a polynomial**
...
serie odot( poly &p2, serie& s1)
{ return(odot(s1,p2)); }

serie odot( gd & gd2, serie& s1)
{   poly p2;
	p2=gd2;
	return(odot(s1,p2)); }

serie odot( serie& s1,  gd & gd2)
{   poly p2;
	p2=gd2;
	return(odot(s1,p2)); }
...
serie fracodotsharp(serie   & num, serie & den)
{
...
if(num.canonise==0) num.canon();
if(den.canonise==0) den.canon();



//** If one series is equal to epsilon

if (num.q.getpol(0).getg()==infinity )
  { result.p=epsilon;
    result.q=epsilon;
	result.r.init(0,0);
	result.canonise=1;
...
if (den.q.getpol(0).getg()==infinity )
  { result.p=epsilon;
    result.q=epsilon;
	result.r.init(0,0);
	result.canonise=1;
...
//** if one series is equal to Top
  ...
  if (num.q.getpol(0).getg()==_infinity || den.q.getpol(0).getg()==_infinity)
  { result.p.init(_infinity,infinity);
    result.q.init(_infinity,infinity);
	result.r.init(0,0);
	result.canonise=1;
...
if (num.r.getg()==0 && den.r.getg()==0)
{ result.p=odot(temp1=oplus(num.p,num.q),temp2=oplus(den.p,den.q));
	result.q=result.p.getpol(result.p.getn()-1);
	result.p.pop();
    if (num.r.getd()==infinity && den.r.getd()==infinity) result.r.init(0,infinity);
	else result.r.init(0,0);
	result.canonise=1;
	return(result);
 }
...
if (num.r.getg()==0 )
 {  result.p=oplus(num.p,num.q);
	result=odot(den,result.p);
	return(result);
 }

if (den.r.getg()==0 )
 {  result.p=oplus(den.p,den.q);
	result=odot(num,result.p);
	return(result);
 }
...
float snum=((float) num.r.getg())/num.r.getd();
float sden=((float) den.r.getg())/den.r.getd();
if (snum<sden)
{
    result.p=epsilon;
    result.q=epsilon;
...
/* the slope computation of the resulting series */
   ...
   a=lcm(num.r.getd(),den.r.getd());
   I=a/num.r.getd();
   J=a/den.r.getd();
   result.r.init(num.r.getg()*I-den.r.getg()*J,a);
...
if(result.q.getn()>1) result.q.pop(); // we remove the last point
else
{ if (!(result.p==epsilon)) result.q=result.p;
}
...
serie fracodotflat(serie   & num, serie & den)
{
